...
const Teachers = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        // fetch groups for each teacher in parallel
        ...
        // derive grade list from teacher groups (unique)
        ...
        teachersWithGroups.forEach((t) => {
          (t.groups || []).forEach((g) => {
            ...
            if (typeof grade === 'object' && grade._id) {
              ...
              const found = gradesData.find(gr => gr._id.toString() === grade.toString());
    ...
    const fetchSubjectsForGrade = async () => {
      if (selectedGrade) {
        try {
          ...
          const subjects = [...new Set(groupsResponse.data?.map(group => group.subject).filter(Boolean))];
  ...
  const filteredTeachers = teachers.filter(teacher => {
    ...
    if (searchQuery) {
      matchesSearch = (teacher.fullName || '').toLowerCase().includes(searchQuery.toLowerCase());
    }

    if (selectedGrade) {
      const teacherGradeIds = (teacher.groups || [])
        .map(g => (g.grade && g.grade._id) ? g.grade._id.toString() : (typeof g.grade === 'string' ? g.grade : null))
        .filter(Boolean);
  ...
  const sliderSettings = {
    ...
    prevArrow: <PrevArrow />,
    responsive: [
      { breakpoint: 1100, settings: { slidesToShow: Math.min(2, filteredTeachers.length) } },
    ...
    <>
      ...
      >
        <div className="max-w-[1400px] mx-auto"> 
          {/* optional: limit carousel content width while keeping full-bleed background; remove wrapper to truly fill */}
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-gray-600 text-sm mt-2">
                          <span className="text-red-600 font-medium">Grades:</span> {getGradesTaught(teacher)}
                        </p>
